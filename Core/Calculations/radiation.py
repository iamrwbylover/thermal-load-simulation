...
def calculateRadiation(fileName):
    global epsi_soil, rhol_soil, rhos_soil, azi, elev, I_dni,I_diff, noOfDays, N, I_dni, I_diff
...
def calculateSW(i, angle):
    ...
    I_dir = I_dni[i]*np.cos(elev[i])*np.cos(angle-azi[i])
    
    if elev[i] < 0:
        I_s= 0
    elif I_dir <0:
        I_dir = 0
        I_sdirh = I_dni[i]*np.sin(elev[i]) + I_diff[i]
        I_srefl = rhos_soil*(I_sdirh + I_diff[i])
        I_s = I_dir + g_atm*I_diff[i] + g_terr*I_srefl
    else:
        I_sdirh = I_dni[i]*np.sin(elev[i]) + I_diff[i]
        I_srefl = rhos_soil*(I_sdirh + I_diff[i])
        I_s = I_dir + g_atm*I_diff[i] + g_terr*I_srefl
